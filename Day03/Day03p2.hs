...
getDirection "R" = \(x,y) -> (x+1, y)
getDirection "L" = \(x,y) -> (x-1, y) 
getDirection "U" = \(x,y) -> (x, y+1) 
getDirection "D" = \(x,y) -> (x, y-1) 

parse :: (Int, Int) -> String -> [(Int, Int)]
parse coords instruction = do
    let direction  = getDirection $ take 1 instruction
    let blocks = read $ drop 1 instruction :: Int
    drop 1 $ take (blocks+1) $ iterate direction coords

getPath :: [String] -> [(Int, Int)]
getPath instructions = concat $ L.scanl' (\path instruction -> parse (last path) instruction) [(0,0)] instructions

wire1path    = getPath wire1instructions
wire2path    = getPath wire2instructions
intersections = S.filter (/= (0,0)) $ S.intersection (S.fromList wire1path) (S.fromList wire2path)

intersectionDistance :: (Int, Int) -> Int
intersectionDistance i = (fromJust $ L.elemIndex i wire1path) + (fromJust $ L.elemIndex i wire2path)

main :: IO()
main = print $ minimum $ S.map intersectionDistance intersections
