...
main :: IO()
main = print $ minimum distances
  where
    wire1path    = S.fromList $ getPath wire1instructions
    wire2path    = S.fromList $ getPath wire2instructions
    intersection = S.filter (/= (0,0)) $ S.intersection wire1path wire2path
    distances    = S.map (\coord -> (abs $ fst coord) + (abs $ snd coord)) intersection
